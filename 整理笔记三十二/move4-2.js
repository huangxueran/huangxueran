...
(function(){
    ...
    function Move(ele , json , time , tween) {
        tween = tween || "bounceIn";

        //初始值的获取
        var CSSObj = ele.currentStyle || getComputedStyle(ele);
        var startVal = {};
        for (var key in json) {
            startVal[key] = parseFloat(CSSObj[key]) || 0;
        }
        ...
        function m(){
            ...
            for (var key in json) {
                var Sx = Tween[tween]( nowDate-startDate , startVal[key] , json[key] - startVal[key] , time );
                ele.style[key] = Sx + 'px';
            }
